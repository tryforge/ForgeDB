...
export default new NativeFunction({
    ...
    version: "2.0.0",
    description: "Creates a member leaderboard of a variable",
    ...
    async execute(ctx) {
        ...
        const rows = await DataBase.allWithType(`${varType}_${(guild?.value as unknown as Guild).id ?? ctx.guild?.id}`, 'member')
