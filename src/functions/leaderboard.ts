...
export default new NativeFunction({
    ...
    async execute(ctx) {
        const [type, valueVariable, positionVariable, code, sortType, max, page, separator] = this.data.fields! as IExtendedCompiledFunctionField[],
            typeExec = (await this["resolveCode"](ctx, type)) as Return
        ...
        const varType = typeExec.value as string,
            pos = positionVariableName.value as string,
            valueName = valueVariableName.value as string,
            sort = sortTypeValue.value === "desc" ? SortType.asc : SortType.desc,
            limit = Number(limitExec.value) || 10,
            pag = Number(pageExec.value) || 1,
            sep = (sepExec.value as string) || "\n",
            elements = new Array<string>(),
            rows = await ForgeDB.allWithType(varType)
